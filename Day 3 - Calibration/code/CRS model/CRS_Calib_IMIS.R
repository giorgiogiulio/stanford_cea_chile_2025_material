...
calc_log_lik <- function(v_params){
  ...
  for (j in 1:n_samp) { # j=1
    jj <- tryCatch( { 
      ...
      # TARGET 1: Survival ("Surv")
      # log likelihood  
      v_llik[j, 1] <- sum(dnorm(x = lst_targets$Surv$value, # [59]
                                mean = model_res$Surv, # [59]
                                sd = lst_targets$Surv$se, # [59]
                                log = T))
...
legend("topright", 
       ...
       col = c("black", "red", "blue"), pch = c(1, 8))
...
m_out_surv_95cri <- colQuantiles(m_out_surv, 
                                 probs = c(0.025, 0.975))
